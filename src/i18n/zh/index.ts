...
const zh = {
  ...
  setting: {
    ...
    imageWidth: {
      label: '导出图片的默认宽度',
      description: '基础宽度：设置导出图片内容的宽度。捕获像素：还会随分辨率缩放和设备 DPR 增加。默认值：750px。',
    },
    padding: {
      title: '图片边框',
      description: '内边距：设置导出图片的内边距。默认值：所有边均为 24px。',
      unified: '统一边距',
      all: '边距',
      top: '上边距',
      right: '右边距',
    ...
    split: {
      ...
      mode: {
        label: '拆分模式',
        description: '拆分模式：选择是否拆分图片。固定高度：每张拆分图片高度固定，可能在拆分处截断文字；按分隔线拆分：按文档中的 hr 元素拆分；按段落自动拆分：让一个段落留在同一张图片中，并尽可能接近拆分高度。',
        none: '不拆分',
        fixed: '固定高度',
        hr: '按分隔线拆分',
        auto: '按段落自动拆分',
      ...
      height: {
        label: '拆分图片高度',
        description: '拆分高度：设置每张拆分图片的高度（像素）。默认值：1000px。',
      },
      overlap: {
        label: '拆分图片重叠',
        description: '重叠区域：设置相邻拆分图片的重叠（像素），防止内容被截断。默认值：80px。',
      },
    },
    filename: {
      label: '是否包含文件名作为标题',
    ...
    resolutionMode: {
      label: '分辨率缩放',
      description:
        '缩放倍率：按设定宽度的倍数渲染导出图片。1x：原始尺寸，文件最小，适合网页和普通显示器；2x：适合 Retina 显示器和社交媒体分享；3x：适合手机等高 PPI 屏幕；4x：适合印刷或需要放大查看的场景。更高倍率：图片更清晰，文件也更大。',
    },
    metadata: {
      label: '显示 metadata',
    },
    format: {
      title: '输出文件格式',
      description:
        'PNG：满足绝大多数普通背景导出需求。透明 PNG：导出透明背景图片；JPG：获得更小的文件，但可能无法直接复制到剪切板；PDF：导出单页 PDF，与常见纸张格式 PDF 不同。',
      png0: 'png - 默认',
      png1: 'png - 导出透明背景图片',
      jpg: 'jpg - 导出 jpg 图片',
      pdf: 'pdf - 导出单页 pdf',
    ...
    assetMark: {
      enable: {
        label: '嵌入隐藏资产标识',
        description: '隐藏资产标识：在导出图片像素中嵌入短标识，便于后续匹配。限制：它不能替代源文件和版权存证。',
      },
      ownerId: {
        label: '资产标识主体 ID',
